...
char *read_index(long offset, FILE *fp) {
    ...
    char *retval = fgets(linep, LINE_LEN, fp);
    if (retval == NULL) {
        fprintf(stderr, "read_index in binsrch.c failed");
        exit(1);
    }
    return(line);
}
...
char *bin_search(char *searchkey, FILE *fp)
{
	...
	char *retval = fgets(linep, LINE_LEN, fp);
    if (retval == NULL) {
        fprintf(stderr, "bin_search in binsrch.c failed");
        exit(1);
    }
...
char *replace_line(char *new_line, char *searchkey, FILE *fp)
{
    ...
    fseek(fp, offset, 0);
    char *retval = fgets(line, LINE_LEN, fp);	/* read original */
    if (retval == NULL) {
        fprintf(stderr, "replace_line in binsrch.c failed");
        exit(1);
    }
    copyfile(fp, tfp);
